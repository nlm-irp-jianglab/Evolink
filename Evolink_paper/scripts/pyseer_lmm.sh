...
#########################################################
# Wrap Pyseer using linear mixed model in a bash script #
#########################################################
...
# distance matrix
python softwares/PySeer/phylogeny_distance.py --lmm ${tree_path} > ${outdir}/phylogeny_K.tsv
...
threshold=`python softwares/PySeer/count_patterns.py --alpha 0.05 --cores 32 --memory 1000 --temp /tmp ${outdir}/output_lmm_patterns.txt|awk 'NR==2'|cut -f2`
echo ${threshold}
awk -F"\t" -v thresh=${threshold} '$4<thresh{print $1}' ${outdir}/output_lmm_patterns.txt > ${outdir}/Pyseer_lmm_sig_genes.list
