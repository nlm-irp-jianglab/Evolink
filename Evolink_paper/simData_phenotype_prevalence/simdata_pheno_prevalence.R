...
# set parameters #
Ntips = 1000
Nsubs = 80 # total number of substitution of characters along the whole tree
prevalence = as.numeric(args[1])
associate_factor = 15 # control the correlation degree for geno and pheno
all_ncpus = 72 #
sim_ncpus = 50 # should be dividable by sim_times
sim_times = 10000 # the times of simmap phenotype on ancestral nodes
N_all_geno = 20000
N_pos_geno = 10
N_neg_geno = 10
...
# outfile
outdir=paste0("prev_", prevalence)

# load tree & pheno
tree = readRDS(paste0(outdir, "/tree.rds"))
pheno = readRDS(paste0(outdir, "/pheno.rds"))
