...
threshold=`python /data/yangy34/softwares/PySeer/count_patterns.py --alpha 0.05 --cores 32 --memory 1000 --temp /tmp ${outdir}/output_fem_patterns.txt|awk 'NR==2'|cut -f2`
echo ${threshold}
awk -F"\t" -v thresh=${threshold} '$4<thresh{print $1}' ${outdir}/output_fem_patterns.txt > ${outdir}/sig_gene.list
