...
gene.sim <- function(tree = NULL, # must be given and must be rooted: if(!is.rooted(tree)) tree <- midpoint(tree)
  ...
  # choose more/less indices to substract/add substitutions acd to dist.prop.bak probability
  m <- sample(1:length(dist.prop.bak), abs(sum(dist.prop.bak) - gen.size), prob=dist.prop.bak, replace=TRUE)
  for(i in m){
    dist.prop.bak[i] = dist.prop.bak[i] - delta
  }
  ...
  #     if(sum(dist.prop.bak) > gen.size){delta = 1}else{delta = -1}
  #     m <- sample(1:length(dist.prop.bak), 1)
  ...
  ## REPLACE ANY NON-POLYMORPHIC LOCI ##
